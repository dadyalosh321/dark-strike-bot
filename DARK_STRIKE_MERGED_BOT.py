...
async def send_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = requests.post(f"https://i.instagram.com/users/{target_id}/flag/", headers=headers, data=data, allow_redirects=False)
    ...
    if res.status_code == 429:
        ...
        if res.status_code == 302:
            await update.message.reply_text(
        "❌ فشل البلاغ: [302 - Redirect]\nغالبًا الجلسة غير صالحة أو الحساب يحتاج تحقق."
    )
        else:
            await update.message.reply_text(
